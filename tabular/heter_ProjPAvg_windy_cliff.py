...
parser.add_argument("--iter", default=64)
...
save_dir = './heter-plot-ProjPAvg-windy_cliff'
...
def experiment():
    ...
    for count in tqdm(range(outer_iter_num)):
        ...
        mix_Ps = generate_mix_windycliff_Ps(n=ntrain + ntest, length_X=lengthX, length_Y=lengthY, eps_lst=eps_lst,
                                            seed=seed)
        ...
        for eps_index in range(eps_lst_len):
            ...
            for E_num in range(E_size - 1):
                ...
                for e in range(args.iter // E):
                    ...
                    if e > 2 and (MEObj_lst[E_num][(e - 1) * E] - MEObj_lst[E_num][(e - 2) * E]) < MEObj_lst[E_num][
                        (e - 2) * E] * 1e-3:
                        lr = lr * args.lr_decay
            ...
            for e in range(args.iter):
                ...
                group_ME_Obj = evaluate_MEObj_from_policy(avg_pi, R, group_P, d_0, gamma)
