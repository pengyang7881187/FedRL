...
from Per_MyHopper import MyHopperEnv as Per_MyHopperEnv
from MyHopper import MyHopperEnv as Ori_MyHopperEnv
...
parser.add_argument("--outer_loop_num", default=50, type=int)
...
save_dir_name = 'plot-PerDDPGAvg-Hopper'
global_log_dir = './PerDDPGAvg-Hopper/'

threshold = 1.0  # This parameter is only for Hopper
...
def Per_DDPG_TRAIN(env: gym.Env,
    ...
    if continue_train_flag:
        act_net = Per_DDPGActor(
            obs_size=Ori_MyHopperEnv().observation_space.shape[0],
...
def exp_diff_merge_interval(env_class: Type[MyEnv],
    ...
    for count in tqdm(range(outer_loop_num)):
        ...
        for k in range(merge_interval_set_size):
            ...
            M_act_net, M_crt_net, _, _ = M_DDPGAvg(env_class=Ori_MyHopperEnv,
                                                   recorder_extra_hyperparameters=recorder_extra_hyperparameters,
                                                   train_theta_set=train_theta_set[count],
                                                   validation_theta_set=validation_theta_set,
                                                   agent_trainer=M_DDPG_TRAIN,
